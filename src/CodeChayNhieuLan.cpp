...
public:
    ...
    void xuLyDaNgua(QuanCo& ta, int vtMoi, NguoiChoi& p, long& countDaNgua) {
...
ds[count++] = i;
    ...
    void diChuyen(NguoiChoi& p, int idx, int dice, long& countDaNgua) {
        QuanCo& n = *p.danhSachNgua[idx];

        if (n.trangThai == TRONG_CHUONG) {
            int start = n.layViTriBatDau();
            xuLyDaNgua(n, start, p, countDaNgua);
        ...
        if (qdMoi < 48) {
            ...
            xuLyDaNgua(n, vtMoi, p, countDaNgua);
            n.viTri = vtMoi;
            banCo[vtMoi] = &n;
        } else if (qdMoi < 54) {
            n.trangThai = LEN_DOC;
...
public:
    ...
    int chonNgua(NguoiChoi& p, DieuKhienTroChoi& game, int dice, int* ds, int size) override {
        ...
        for (int i = 1; i < size; i++) {
            if (p.danhSachNgua[ds[i]]->quangDuongDaDi > p.danhSachNgua[best]->quangDuongDaDi) best = ds[i];
        }
        return best;
    }
};
...
public:
    string ten() override { return "Can Bang"; }
    int chonNgua(NguoiChoi& p, DieuKhienTroChoi& game, int dice, int* ds, int size) override {

        if (dice == 6 || dice == 1) {
            for (int i = 0; i < size; i++) {
                QuanCo& n = *p.danhSachNgua[ds[i]];
                if (n.trangThai == TRONG_CHUONG) return ds[i];
            }
...
void runMassSimulation(int n) {
    long wins[4] = {0, 0, 0, 0};
    long totalDaNgua[4] = {0, 0, 0, 0};
    long totalBonus[4] = {0, 0, 0, 0};
...
if (size > 0) {
                        long beforeDaNgua = totalDaNgua[j];

                        int choice = players[j]->chienThuat->chonNgua(*players[j], game, dice, ds, size);
                        game.diChuyen(*players[j], choice, dice, totalDaNgua[j]);

                        if (totalDaNgua[j] > beforeDaNgua) totalBonus[j]++;

                        if (players[j]->danhSachNgua[choice]->trangThai == VE_DICH) totalBonus[j]++;
                    }
    ...
    string filename = "MoPhong_" + to_string(n) + "van.csv";
    ofstream outFile(filename);

    outFile << "Chien Thuat,So Van Thang,Ty Le Thang (%),Da Ngua TB,Bonus TB\n";
    string names[] = {"Hung Hang", "Uu Tien Ve Dich", "Dan Quan", "Can Bang"};

    for (int i = 0; i < 4; i++) {
        outFile << names[i] << ","
                << wins[i] << ","
                << fixed << setprecision(2) << ((double)wins[i] / n) * 100 << "%,"
                << ((double)totalDaNgua[i] / n) << ","
                << ((double)totalBonus[i] / n) << "\n";
    }

    outFile.close();
    cout << "  -> Hoan tat " << n << " van. Luu vao: " << filename << endl;
}

int main() {
    srand(time(0));

    cout << "=== MO PHONG CO CA NGUA TIEU CHUAN ===" << endl;

    cout << "Dang chay 1,000 van..." << endl;
    runMassSimulation(1000);

    cout << "Dang chay 10,000 van..." << endl;
    runMassSimulation(10000);

    cout << "Dang chay 100,000 van (Doi vai giay)..." << endl;
    runMassSimulation(100000);

    cout << "HOAN TAT TASK 3! XEM KET QUA TAI THU MUC CHUA CODE." << endl;
    return 0;
}
